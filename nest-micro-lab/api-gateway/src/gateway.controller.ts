...
export class GatewayController {
  ...
  async routeAll(@Req() req: express.Request, @Res() res: express.Response) {
    ...
    if (!routeConfig) {
      console.warn(
        `[Gateway] Unknown route requested: ${serviceKey} requestId=${requestId}`,
      );
    ...
    if (routeConfig.rateLimit) {
      ...
      const allowed = rateLimiter.consume(
        key,
        routeConfig.rateLimit.points,
        routeConfig.rateLimit.windowMs,
      );
      if (!allowed) {
        console.warn(
          `[Gateway] Rate limit exceeded: ${serviceKey} ip=${ip} requestId=${requestId}`,
        );
        return res
          .status(429)
    ...
    const upstream = await this.proxy.forward({
      ...
      query: req.query,
      body: req.body,
      timeout: routeConfig.timeout ?? DEFAULT_TIMEOUT,
